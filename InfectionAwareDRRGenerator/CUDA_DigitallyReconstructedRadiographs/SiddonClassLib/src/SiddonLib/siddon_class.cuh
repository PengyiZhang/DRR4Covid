...
*
* Based both on the description found in the 揑mproved Algorithm?section in Jacob抯 paper (1998)
...
public:
	...
	SiddonGpu(int *NumThreadsPerBlock,
			  float *movImgArray,
			  float *movMaskArray, // 用于施加指示位置
			  float *Weights, // 用于施加指示权重
			  int *MovSize,
			  float *MovSpacing,
...
private:
	...
	float *m_d_movMaskArray; // device copy
	float *m_d_Weights; // device copy
